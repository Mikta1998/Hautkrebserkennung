...
def train_yolo(yolo_path: str, data_path: str, epochs: int = 100, patience: int = 10):
    ...
    for epoch in range(epochs):
        # Training step
        model.train(data=data_path, epochs=100, plots=False)
        
        # Validation step (assuming val data is in 'val' subfolder)
        metrics = predicition_yolo(yolo_path, os.path.join(data_path, 'val'))
        val_loss = 1 - (metrics[1] / 100)  
...
def main(data_path_predict, save_results=False, results_path='None', train=False, data_path_train='None', epochs=100, patience=10):
    start_time = time.strftime("%d%m-%H%M")

    YOLO_PATH = r'F:\KI in den Life Sciences\hautkrebserkennung\YOLO\models\yolov8m-cls.pt'

    if train:
        train_yolo(YOLO_PATH, data_path_train, epochs, patience)
    else:
...
if __name__ == '__main__':
    ...
    main(args.image_path, args.save_results, args.results_path, args.train, args.data_path, args.epochs, args.patience)